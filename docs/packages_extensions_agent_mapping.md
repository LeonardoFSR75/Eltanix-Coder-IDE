...
```
         ...
         │ StatusBar (indicador) │  window  │ eltanix:packages:changed│
         └────────────────────────┘  event  │ eltanix:extensions:...  │
                                              └────────────┬────────────┘
                                                            │
                                              ┌─────────────▼────────────┐
                                              │ PackagesCard.tsx          │
...
- `extensions/client.py` (`OpenVSXClient`) — busca no Open VSX Registry (`search_marketplace`,
  ...
  10 min com chave `eltanix:cache:extensions:search:<sha256(query)>` (task #8) — sem Redis,
...
`.eltanix/worktrees/`) antes de agir, porque a sessão do agente frequentemente roda numa
...
`eltanix:browser:open` quando a ferramenta `browser_action` sucede. Estendido no mesmo padrão:
...
if (
  ...
  const eventName = message.name === "manage_packages"
    ? "eltanix:packages:changed"
    : "eltanix:extensions:changed";
...
| Frontend — painel de pacotes | `apps/web/components/ide/PackagesPanel.test.tsx` | render de lista/vazio/erro, confirmação in-app antes de desinstalar, evento `eltanix:packages:changed` disparado após install e reagido quando disparado externamente |
| Frontend — painel de extensões | `apps/web/components/ide/ExtensionsPanel.test.tsx` | render de lista, erro+retry (`PanelState`), toggle ativo/inativo, evento `eltanix:extensions:changed` reagido quando disparado externamente |
