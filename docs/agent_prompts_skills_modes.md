...
`\n\n## Instruções do projeto\n\n{custom_instructions}` (se `.eltanix/instructions.md`
...
### 4.1 Skills do agente (banco de dados, `eltanix.skills.*`)
...
e, se `workspace_root` existir, também em `.eltanix/skills/<slug>.md` — mas esse arquivo de
projeto **não é relido em nenhum lugar** (não há `rglob` sobre `.eltanix/skills/` em nenhum
...
de skills do agente Eltanix Coder IDE descrito acima. Os dois compartilham o nome "skill" e o formato
`SKILL.md`, mas são pipelines completamente separados (um roda dentro do Claude Code que edita
este repositório; o outro roda dentro do agente que o Eltanix Coder IDE expõe aos usuários finais).

## 5. Aprovação e política (`agent/approval_policy.py`)

Opt-in, por projeto, guardada em `.eltanix/approval_policy.yaml`. Duas formas de regra:
...
- **Instruções do projeto** (`.eltanix/instructions.md`, editável na aba "Instruções" do
...
| Camada | Arquivo |
| --- | --- |
| System prompt + prompt por modo | `apps/api/src/eltanix/agent/prompts.py` |
| Ciclo think/approve/act, gate de ferramentas por modo | `apps/api/src/eltanix/agent/graph.py` |
| Política de auto-aprovação | `apps/api/src/eltanix/agent/approval_policy.py` (+ `approval_policy_config.py`, `approval_policy_editor.py`) |
| Ferramentas de skills do agente | `apps/api/src/eltanix/agent/tools/skills.py` |
| Ferramenta de plano/checklist | `apps/api/src/eltanix/agent/tools/plan.py` |
| Serviço/store de skills (Postgres) | `apps/api/src/eltanix/skills/service.py`, `store.py` |
| Seed de skills a partir de `.agents/` | `apps/api/src/eltanix/skills/seed.py` |
