...
Docker-interno (`sicoobito-<session_id>`, `host.docker.internal`) porque o hostname original não resolve
...
**Validação SSRF compartilhada.** `sicoobito.security.url_safety` é o módulo único consumido por
...
serviço roda isolado, sem o pacote `sicoobito` instalado, para manter a menor superfície possível numa
