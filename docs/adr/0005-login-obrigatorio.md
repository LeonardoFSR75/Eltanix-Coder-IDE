...
`127.0.0.1`" + `ELTANIX_API_KEY` compartilhada (`api/deps.py::require_api_key`),
...
- `ELTANIX_API_KEY` continua existindo e validando por `hmac.compare_digest`,
...
nada se `app_user` já tem alguém. Sem `ELTANIX_ADMIN_PASSWORD` no `.env`,
