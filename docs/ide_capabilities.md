# 🛠️ Capacidades e Estratificação da IDE Agêntica (SicoobitoCode)

Este documento estratifica a arquitetura, ferramentas, classes de risco, fluxos de trabalho e restrições da **IDE Agêntica Local-First SicoobitoCode** por tipo de funcionalidade.
...
- **Gate de LLM Único ([ADR 0001](file:///c:/Users/leona/Documents/Projetos/SicoobitoCode/docs/adr/0001-camada-unica-de-llm.md))**: NENHUM módulo consome APIs diretas de provedores (OpenAI, Anthropic, Gemini). Todo o tráfego passa obrigatoriamente por `sicoobito.router` (`RouterEngine.complete()` / `.embed()`).
...
- **Autenticação Obrigatória ([ADR 0005](file:///c:/Users/leona/Documents/Projetos/SicoobitoCode/docs/adr/0005-login-obrigatorio.md))**: Toda rota HTTP exige `AuthDep` (sessão via cookie HttpOnly ou chave `SICOOBITO_API_KEY`).
