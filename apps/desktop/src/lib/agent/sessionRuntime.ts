/**
 * Motor de uma sessão do agente, sem hook/rune — porta de
 * `apps/web/components/ide/agent/sessionRuntime.ts`.
 *
 * Fica fora de componente de propósito: o Agent Manager roda N sessões em
 * paralelo, e cada `AgentSessionRuntime` é um objeto plano com os campos de
 * estado; quem quiser reagir a mudanças assina `onChange`. Componentes Svelte
 * espelham esses campos em `$state` local dentro do callback — ver
 * `AgentDock.svelte`/`AgentManager.svelte`.
 *
 * IMPORTANTE: a aprovação de ferramentas WRITE/EXEC não é uma rota própria —
 * é reenviada como `approvals` no mesmo POST que retoma o stream da sessão
 * (`/api/agent/sessions/{id}/run`). Não existe `/api/agent/approve`.
 */

import { get, post, streamEvents } from "../client";
...
export class AgentSessionRuntime {
  ...
  running = false;
  todos: TodoItem[] = [];
  /** Sessão carregada do histórico (via /messages), sem stream ativo. */
  readOnly = false;
  finished = false;
  errored = false;
  ...
  async run(approvals?: Record<string, { approved: boolean; reason: string }>) {
    // Sem esta guarda, aprovar seguido de recusar antes do card sumir dispara
    // duas chamadas concorrentes, e a segunda sobrescreve `abortController` da
