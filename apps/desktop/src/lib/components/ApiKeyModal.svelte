  ...
  >
    <div class="modal-card" role="document" onclick={(e) => e.stopPropagation()}>
      <div class="modal-header">
        <h3>🔑 Chave de API do Eltanix Coder IDE</h3>
        {#if dismissible}
      ...
      <p class="hint">
        O Lite fala direto com a API (<code>ELTANIX_API_KEY</code>) — sem chave, nenhuma
      ...
      <form onsubmit={handleSubmit} class="modal-form">
        <label>
          ...
          <input
            type="password"
            bind:value={key}
            placeholder="cole a ELTANIX_API_KEY aqui..."
            autocomplete="off"
            required
          />
        </label>
