...
describe("AgentChatInput — slash commands reais", () => {
  ...
  it("Enter com o menu aberto seleciona o comando em vez de enviar a mensagem", async () => {
    ...
    const textarea = await screen.findByPlaceholderText(/Pergunte ao Sicoobito Agente/);
...
const WORKSPACE_FILES = [
  { path: "apps/api/src/sicoobito/main.py" },
  { path: "apps/api/src/sicoobito/agent/runner.py" },
...
describe("AgentChatInput — menções @ de contexto (Fase 5)", () => {
  ...
  it("mostra @docs, @web e arquivos que casam ao digitar '@'", async () => {
    ...
    expect(screen.getByText("apps/api/src/sicoobito/main.py")).toBeInTheDocument();
  });

  it("filtra arquivos pelo termo digitado após o '@'", async () => {
    renderInput({ task: "corrija o @runner" });
    await screen.findByRole("listbox");
    expect(screen.getByText("apps/api/src/sicoobito/agent/runner.py")).toBeInTheDocument();
  ...
  it("clicar num arquivo insere o caminho no texto e anexa aos arquivos em foco", async () => {
    ...
    await userEvent.click(screen.getByText("apps/api/src/sicoobito/agent/runner.py"));

    expect(setTask).toHaveBeenCalledWith("corrija o @apps/api/src/sicoobito/agent/runner.py ");
    expect(setFocusFiles).toHaveBeenCalled();
    const updater = setFocusFiles.mock.calls[0][0] as (prev: string[]) => string[];
    expect(updater([])).toEqual(["apps/api/src/sicoobito/agent/runner.py"]);
  });
