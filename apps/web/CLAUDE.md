...
E2E_PASSWORD=<mesma senha de ELTANIX_ADMIN_PASSWORD no .env> npm run test:e2e
```

Sem `ELTANIX_ADMIN_PASSWORD` fixado no `.env`, a API gera uma senha aleatória por
...
- **Regra para Agentes**: Sempre consultar os MOCs em `graphify-out/obsidian/00 - 🏠 Painel & MOCs/` e o `Mapa Arquitetural Eltanix Coder IDE.canvas` para manter alinhamento com a arquitetura geral da UI.
