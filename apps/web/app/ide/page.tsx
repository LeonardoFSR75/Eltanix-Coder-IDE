...
function Shell() {
  ...
  const [linkModalTab, setLinkModalTab] = useState<"link" | "create" | "clone" | null>(null);
  ...
  useEffect(() => {
    const handleBrowserOpen = (evt: Event) => {
      ...
      if (url) {
        ...
        // serviço de navegador (`eltanix-<sessionId>:<porta>`).
        ide.openFile(`browser-agent:${url}`);
      }
    };
    window.addEventListener("eltanix:browser:open", handleBrowserOpen);
    return () => window.removeEventListener("eltanix:browser:open", handleBrowserOpen);
  }, [sessionId, ide]);
  ...
  return (
    ...
    >
      ...
      <div className={`ide${ide.agentDockOpen ? " agent-dock-open" : ""}`}>
        ...
        {linkModalTab && (
          <LinkProjectModal
            initialTab={linkModalTab}
            onClose={() => setLinkModalTab(null)}
