...
Usa o canal de serviço (SICOOBITO_API_KEY, ver docs/adr/0005-login-obrigatorio.md)
em vez de login de usuário — é ferramenta externa, não sessão de browser.

Uso:
  SICOOBITO_API_KEY=... uv run python scripts/load_test_sandbox_queue.py \
...
async def main() -> int:
    ...
    api_key = os.environ.get("SICOOBITO_API_KEY", "")
    if not api_key:
        print("SICOOBITO_API_KEY não definida no ambiente.", file=sys.stderr)
        return 1
