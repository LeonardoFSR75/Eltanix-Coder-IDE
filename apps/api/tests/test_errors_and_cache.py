...
from eltanix.router.errors import FailureKind, classify, extract_failed_tool_call
...
def test_extract_failed_tool_call_recovers_groq_pseudo_xml_format():
    exc = _groq_tool_use_failed(
        '<function=list_files{"path": "apps/api/src/eltanix/agent/tools"}</function>'
    )
    assert extract_failed_tool_call(exc) == (
        "list_files",
        {"path": "apps/api/src/eltanix/agent/tools"},
    )
