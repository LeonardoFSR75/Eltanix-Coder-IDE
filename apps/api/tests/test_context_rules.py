...
from sicoobito.agent.context_rules import (
    ContextRule,
    ContextRulesConfig,
...
from sicoobito.agent.runner import _load_context_rules_prompt
...
class TestMatchContextRules:
    ...
    def test_matches_focus_file(self):
        ...
        resultado = match_context_rules(
            config, focus_files=["apps/api/src/sicoobito/main.py"], focus_folder=None
        )
...
class TestLoadContextRules:
    ...
    def test_reads_valid_yaml(self, tmp_path):
        (tmp_path / ".sicoobito").mkdir()
        (tmp_path / ".sicoobito" / "context_rules.yaml").write_text(
