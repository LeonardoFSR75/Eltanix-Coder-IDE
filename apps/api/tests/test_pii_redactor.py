...
from eltanix.security.pii_redactor import PIIRedactor
...
def test_pii_redactor_email():
    text = "Enviar relatórios para leandro.admin@eltanix.com.br imediatamente."
    res = PIIRedactor.redact(text)
    assert "[REDACTED_EMAIL]" in res.sanitized_text
    assert "leandro.admin@eltanix.com.br" not in res.sanitized_text
    assert res.redacted_count == 1
