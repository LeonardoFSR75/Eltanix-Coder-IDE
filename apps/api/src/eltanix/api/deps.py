...
from eltanix.db.session import get_session
...
def require_api_key(
    ...
    Sem `ELTANIX_API_KEY` definida a API fica aberta — aceitável para uso
...
async def require_session(
    ...
    authorization: Annotated[str | None, Header()] = None,
    x_api_key: Annotated[str | None, Header()] = None,
    eltanix_session: Annotated[str | None, Cookie()] = None,
) -> None:
...
def identify_source(
    x_eltanix_source: Annotated[str | None, Header()] = None,
...
def identify_project(
    x_eltanix_project: Annotated[str | None, Header()] = None,
) -> str | None:
    ...
    projeto do Eltanix Coder IDE e simplesmente não mandam o header — nesse caso o
    custo fica sem projeto associado, igual acontecia antes desta dependency existir.
    """
    return x_eltanix_project[:128] if x_eltanix_project else None
