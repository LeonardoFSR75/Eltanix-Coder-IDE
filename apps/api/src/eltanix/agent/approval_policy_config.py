...
from eltanix.agent.approval_policy import ApprovalPolicy
from eltanix.logging_setup import get_logger

log = get_logger(__name__)

_RELATIVE_PATH = Path(".eltanix") / "approval_policy.yaml"


def load_approval_policy(workspace_root: Path) -> ApprovalPolicy:
    """Lê `.eltanix/approval_policy.yaml` do projeto (não do worktree da
    sessão — mesma raiz que `.eltanix/instructions.md`, ver
