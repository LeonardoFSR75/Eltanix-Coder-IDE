...
from eltanix.sandbox.container import SandboxError
...
def project_venv_prefix(workspace_root: str | Path | None) -> str:
    ...
    while cur != cur.parent:
        if cur.name == "worktrees" and cur.parent.name == ".eltanix":
...
async def run_command(ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
    ...
    if eh_comando_pacote and not network_enabled:
        return ToolResult(
            ...
            content=(
                "[saída 1 em 0ms]\n"
                "ERRO DE AMBIENTE: O sandbox do Eltanix Coder IDE está isolado da rede por segurança.\n"
    ...
    if eh_servidor_provavel and not eh_comando_pontual:
        ...
        comando = (
            ...
            f"sleep 0.2; "
            f"nohup bash -c '{cmd_escapado}' > /tmp/eltanix_server.log 2>&1 & "
            f"for i in $(seq 1 20); do "
            f"(echo > /dev/tcp/127.0.0.1/{porta_alvo}) >/dev/null 2>&1 && break || sleep 0.3; "
            f"done; "
            f"sleep 0.3; "
            f"cat /tmp/eltanix_server.log"
        )
