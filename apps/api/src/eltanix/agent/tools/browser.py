...
from eltanix.browser.client import BrowserError, BrowserUnavailableError
from eltanix.security.url_safety import is_agent_local_test_target
...
@tool(
    ...
    description=(
        ...
        "rodando (a própria interface do Eltanix Coder IDE, por exemplo). Ações: `navigate` "
    ...
    if acao == "navigate":
        ...
        if not is_agent_local_test_target(hostname):
            ...
            return ToolResult(
                ok=False,
                content=(
                    "ERRO: AVISO DE AMBIENTE: O navegador do Eltanix Coder IDE é isolado da rede pública e "
